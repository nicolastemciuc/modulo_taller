...
run_one() {
  ...
  need "$SPARK_SUBMIT"
  local jar; jar="$(find_jar "$subdir")"; [[ -n "$jar" ]] || die "Jar not found under $subdir/target"
  local run_dir="$TRACE_ROOT/$app/$(ts_dir)"; mkdir -p "$run_dir"
  ( "$SPARK_SUBMIT" \
      --class "$klass" \
      --master "$MASTER_URL" \
      ...
      "$jar" $args ) >"$run_dir/spark.out" 2>"$run_dir/spark.err" &
  local pid=$!; echo "$pid" >"$run_dir/driver.pid"; echo "$pid" >>"$PID_FILE"
...
mkdir -p "$(dirname "$PID_FILE")"; : > "$PID_FILE"
